...
def upgrade() -> None:
    ...
    # 2. Enums
    op.execute("CREATE TYPE oauth_provider AS ENUM ('x', 'google', 'local', 'other')")
    op.execute("CREATE TYPE unit_type AS ENUM ('weight', 'volume', 'count', 'other')")
    op.execute("CREATE TYPE recipe_status AS ENUM ('draft', 'published')")
    op.execute("CREATE TYPE storage_provider AS ENUM ('minio', 's3')")
    ...
    op.create_table('units',
        ...
        sa.Column('unit_type', sa.Enum('weight', 'volume', 'count', 'other', name='unit_type', create_type=False), server_default='other', nullable=False),
    ...
    op.create_table('oauth_accounts',
        ...
        sa.Column('provider', sa.Enum('x', 'google', 'local', 'other', name='oauth_provider', create_type=False), nullable=False),
    ...
    op.create_table('recipes',
        ...
        sa.Column('status', sa.Enum('draft', 'published', name='recipe_status', create_type=False), server_default='draft', nullable=False),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_table('recipe_media',
        ...
        sa.Column('storage_provider', sa.Enum('minio', 's3', name='storage_provider', create_type=False), nullable=False),
        sa.Column('bucket', sa.Text(), nullable=False),
        sa.Column('object_key', sa.Text(), nullable=False),
        sa.Column('media_type', sa.Enum('image', 'video', name='media_type', create_type=False), nullable=False),
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE storage_provider")
