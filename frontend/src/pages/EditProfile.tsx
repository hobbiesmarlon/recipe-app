...
const EditProfile: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 2. Update user profile or Register
      if (pendingUser) {
        // First-time registration (Bulletproof Flow)
        await client.post('/auth/cognito-register', {
          token,
          username,
  ...
  const isXUser = user && !user.cognito_sub && isSourcedFromProvider;
    ...
    <div className="flex min-h-screen flex-col bg-background-light dark:bg-background-dark">
      <header className="py-4 sticky top-0 md:top-14 z-10 bg-background-light dark:bg-background-dark">
        <div className="mx-auto max-w-2xl px-4 flex items-center justify-between relative">
          <button onClick={handleCancel} className="p-3 flex items-center gap-3 text-primary z-20 md:-ml-8 lg:hidden -ml-2 rounded-full hover:bg-primary/10 transition-colors">
          ...
          <span className="font-bold text-black dark:text-white absolute left-0 right-0 text-center pointer-events-none md:hidden">
            {isPending ? 'Complete Registration' : 'Edit Profile'}
          </span>
        ...
        <section className="pt-6">
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="relative">
              ...
              {(!isSourcedFromProvider || isPending || user?.cognito_sub) && (
        ...
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          <div>
            ...
            <input 
              ...
              disabled={(isXUser && !isPending) || isLoading}
              placeholder="e.g. Chef Marlon"
      ...
      <footer className="fixed bottom-0 left-0 right-0 border-t border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark pb-safe md:static md:border-none md:mt-10">
        <nav className="mx-auto max-w-2xl flex h-20 items-center justify-between px-4">
          ...
          <button onClick={handleSave} disabled={isLoading || (isXUser && !isPending)} className="rounded-xl bg-primary px-8 py-3 text-sm font-bold text-white hover:bg-orange-600 shadow-lg shadow-primary/20 transition-all active:scale-95 disabled:opacity-50">
            {isLoading ? 'Saving...' : (isPending ? 'Finish Sign up' : 'Save Changes')}
          </button>
        </nav>
      </footer>
